...
function drawCld(SY, SM) {
    var holidaysDayArray = dateSelection.GetholidaysDay(SY,SM);
    var i,sD,s,size;
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            if (holidaysDayArray.indexOf(sD + 1) > -1) {
                sObj.style.color = "red";  //节假日颜色
                lObj.style.color = "red";
                if (lObj.children.length > 0) {
                    lObj.children[0].style.color = "red";
                }
                //$("#GD" + i).addClass("holidayCss");
...
function mOck(thisObj, v){
    ...
    if(day!="")
    {
        ...
        dayJson = {HolidaysDate:nian+"-"+addZ(yue)+"-"+addZ(day)};
        if(onoff == '0'){
...
function delArry(arr,obj){
    for (var i = arr.length - 1; i > -1; i--) { 
        if (arr[i].HolidaysDate == obj.HolidaysDate) { 
