...
//
	...
	*认证(登录时调用)
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		//查询用户信息
		SysUserEntity user = sysUserDao.findUser(token.getUsername());
		...
		/*设置角色标识*/
		List<SysRoleEntity> listRole = sysRoleDao.getRolesfromUserId(user.getUserId());
		String roleStr="";
		for(SysRoleEntity sr:listRole)
		{
			if(roleStr.equals(""))
			{
				roleStr = sr.getRoleKey();
			}
			else
			{
				roleStr = roleStr + sr.getRoleKey();
			}
		}
